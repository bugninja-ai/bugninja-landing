  ...
  return (
    <div className="space-y-8">
      ...
      <div className="space-y-4">
        <h3 className="text-lg font-medium">{t('feedback.alerts.title')}</h3>
        <div className="space-y-4">
          <Alert variant="default">
            <CheckCircle2 className="h-4 w-4" />
            <AlertTitle>{t('feedback.alerts.success.title')}</AlertTitle>
            <AlertDescription>{t('feedback.alerts.success.description')}</AlertDescription>
          </Alert>

          <Alert variant="destructive">
            <AlertCircle className="h-4 w-4" />
            <AlertTitle>{t('feedback.alerts.error.title')}</AlertTitle>
            <AlertDescription>{t('feedback.alerts.error.description')}</AlertDescription>
          </Alert>

          <Alert variant="warning">
            <AlertTriangle className="h-4 w-4" />
            <AlertTitle>{t('feedback.alerts.warning.title')}</AlertTitle>
            <AlertDescription>{t('feedback.alerts.warning.description')}</AlertDescription>
          </Alert>

          <Alert variant="info">
            <Info className="h-4 w-4" />
            <AlertTitle>{t('feedback.alerts.info.title')}</AlertTitle>
            <AlertDescription>{t('feedback.alerts.info.description')}</AlertDescription>
          </Alert>
        </div>
      </div>
      ...
      {/* Toasts */}
      <div className="space-y-4">
        <h3 className="text-lg font-medium">{t('feedback.toast.title')}</h3>
        <div className="flex gap-4">
          ...
          >
            {t('feedback.toast.showToast')}
          </Button>
          <Button variant="secondary" onClick={() => toast({
            title: t('feedback.toast.hook.title'),
            description: t('feedback.toast.hook.description'),
          })}>
            {t('feedback.toast.hookToast')}
          </Button>
        </div>
      </div>
